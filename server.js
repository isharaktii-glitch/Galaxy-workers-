...
const app = express();
// 🔑 SUPABASE CONFIGURATION (ඔයාගේ අලුත් Cloud Database එකට කෙලින්ම Connect වෙනවා)
...
return <!DOCTYPE html><html lang="${lang}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${title}</title> <style> body{background:#0b0c10;color:#c5c6c7;font-family:sans-serif;padding:20px;margin:0;}  .container{max-width:800px;margin:30px auto;background:#1f2833;padding:25px;border-radius:10px;border:1px solid #45a29e;box-shadow: 0px 0px 15px rgba(69, 162, 158, 0.2);position:relative;} .lang-selector { position: absolute; top: 15px; right: 15px; } .lang-selector select { background: #0b0c10; color: #66fcf1; border: 1px solid #45a29e; padding: 5px 10px; border-radius: 5px; cursor: pointer; font-weight: bold; } input, select, textarea {width:95%;padding:10px;margin:8px 0;border-radius:5px;border:1px solid #45a29e;background:#0b0c10;color:#fff;}  button{width:100%;padding:12px;background:#45a29e;border:none;color:#0b0c10;font-weight:bold;font-size:16px;border-radius:5px;cursor:pointer;margin-top:10px;} button:hover{background:#66fcf1;} .user-row{background:#0b0c10;padding:12px;margin:10px 0;border-radius:5px;border-left:5px solid #45a29e;text-align:left;} .cpa-card{background:#141d26; padding:15px; margin:15px 0; border-radius:8px; border:1px solid #66fcf1; text-align:left;} a{color:#66fcf1;text-decoration:none;} .logout-btn{background:#ff4d4d;color:#fff;width:auto;padding:5px 10px;font-size:12px;float:right;margin-left:10px;} .delete-btn{background:#ff4d4d; color:#fff; padding:5px 10px; border:none; border-radius:3px; cursor:pointer; float:right; font-size:12px;} </style></head><body><div class="container"> <div class="lang-selector"> <select onchange="window.location.href='/change-lang?lang=' + this.value}"> <option value="en" ${lang === 'en' ? 'selected' : ''}>English</option> <option value="si" ${lang === 'si' ? 'selected' : ''}>සිංහල</option> <option value="ta" ${lang === 'ta' ? 'selected' : ''}>தமிழ்</option> </select> </div><h2 style="text-align:center;color:#66fcf1;margin-top:15px;">${translations[lang].title}</h2>${content}</div></body></html>;
};
// LANGUAGE CHANGE
...
// REGISTER ACTION (SUPABASE FIXED)
...
const t = translations[req.session.lang || 'en'];
res.send(htmlWrapper(req, 'Forgot Password', <h3>${t.recoverTitle}</h3> <form action="/forgot-password" method="POST"> <input type="text" name="username" placeholder="${t.user}" required> <input type="email" name="email" placeholder="${t.email}" required> <button type="submit">${t.btnRecover}</button> </form> <p style="text-align:center;"><a href="/">${t.backLog}</a></p>));
});
app.post('/forgot-password', async (req, res) => {
const { username, email } = req.body;
const { data, error } = await supabase.from('users').select('password').eq('username', username).eq('email', email).maybeSingle();
if (data) {
res.send(htmlWrapper(req, 'Password Recovered', <div style="border:1px solid #45a29e; padding:20px; border-radius:5px; text-align:center;"> <h3 style="color:#66fcf1;">Your Password is:</h3> <h1 style="color:#fff; background:#0b0c10; padding:10px; display:inline-block; border-radius:5px;">${data.password}</h1> <br><br><a href="/">Click here to Login</a> </div>));
} else {
...
const t = translations[req.session.lang || 'en'];
res.send(htmlWrapper(req, 'Change Password', <h3>${t.changePassTitle}</h3> <form action="/change-password" method="POST"> <input type="password" name="oldPassword" placeholder="${t.oldPass}" required> <input type="password" name="newPassword" placeholder="${t.newPass}" required> <button type="submit">${t.btnUpdate}</button> </form> <p style="text-align:center;"><a href="/dashboard"><- Back to Dashboard</a></p>));
});
app.post('/change-password', async (req, res) => {
if (!req.session.user) return res.redirect('/');
const { oldPassword, newPassword } = req.body;
const username = req.session.user.username;
const { data, error } = await supabase.from('users').select('password').eq('username', username).maybeSingle();
...
const timewallIframe = <h3 style="color:#66fcf1; margin-top:30px; text-align:left;">${t.tasks}</h3> <p style="font-size:13px; color:#aaa; text-align:left;">${t.subText}</p> <iframe src="${finalIframeSrc}" width="100%" height="600px" frameborder="0" style="border-radius:8px; background:#fff; margin-top:15px;"></iframe>;
if (user === 'admin') {
...
let workerList = (users || []).map(u => <div class="user-row"> <a href="/admin/delete-user/${u.id}" class="delete-btn" onclick="return confirm('Are you sure you want to remove this worker?')">Remove Worker</a> <strong>Worker:</strong> ${u.username} | <strong>Email:</strong> ${u.email} <br> <strong>Balance:</strong> $${parseFloat(u.balance).toFixed(4)} | <strong>Password:</strong> <span style="color:#66fcf1;">${u.password}</span> <br> <strong>Address:</strong> ${u.address} | <strong>Contact:</strong> ${u.contact} </div>).join('');
let activeOffersList = (offers || []).map(o => <div class="user-row" style="border-left-color: #ff4d4d;"> <a href="/admin/delete-cpa/${o.id}" class="delete-btn">Delete Offer</a> <strong>[${o.network_name}]</strong> ${o.offer_title} - <span style="color:#66fcf1;">$${o.payout}</span><br> <small style="word-break: break-all;">Link: ${o.offer_link}</small> </div>).join('');
...
const { data: userData } = await supabase.from('users').select('balance').eq('username', user).maybeSingle();
...
return <div class="cpa-card"> <h4 style="margin:0 0 5px 0; color:#66fcf1;">${o.offer_title}</h4> <p style="margin:5px 0; font-size:14px;"> <strong>${t.instructionTitle}</strong>  <span class="translate-text">${o.instruction_en}</span> </p> <p style="margin:5px 0; color:#ff4d4d; font-weight:bold;">Reward: $${parseFloat(o.payout).toFixed(4)}</p> <a href="${trackingLink}" target="_blank"><button style="padding:8px; font-size:14px; margin-top:5px;">👉 Complete Task</button></a> </div>;
}).join('');
...
`;
cpaSection = <h3 style="color:#66fcf1; text-align:left; margin-top:25px;">${t.cpaTasks}</h3> ${offersHtml} ${translationScript};
}
...
let logList = (logs || []).map(l => <div class="user-row log-item" data-username="${l.username.toLowerCase()}" data-task="${l.task_name.toLowerCase()}" style="border-left-color: #66fcf1;"> <strong>Worker:</strong> <span class="worker-name">${l.username}</span> <br> <strong>Task Details:</strong> ${l.task_name} <br> <strong>Earned (After 30% Profit Cut):</strong> <span style="color:#66fcf1;">$${parseFloat(l.amount).toFixed(4)}</span> <br> <small style="color:#aaa;">Time: ${l.timestamp}</small> </div>).join('');
const searchScript = `
...
`;
res.send(htmlWrapper(req, 'Task Logs', <a href="/dashboard" style="font-size:14px;"><- Back to Control Panel</a> <h2 style="color:#66fcf1; margin-top:15px;">Completed Task Logs</h2> ${searchScript} <div id="logsContainer" style="margin-top:10px;"> ${logList || '<p style="text-align:center;color:#888;">No tasks completed yet.</p>'} </div>));
});
// POSTBACK
...
const { data: user } = await supabase.from('users').select('balance').eq('username', subid).maybeSingle();
if (user) {
