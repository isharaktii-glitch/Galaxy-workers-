    ...
    if (username === 'admin') {
        ...
        res.send(htmlWrapper(req, 'Worker Dashboard', `
            ...
            <div id="worker-tasks" class="dashboard-section active">
                <h3>Balance: $${user.balance.toFixed(2)}</h3>
                ${cpaTasksHtml}
            </div>
...
app.post('/send-notification', (req, res) => {
    ...
    notificationsTable.push({ id: notificationsTable.length + 1, target_user, message, timestamp: new Date().toISOString() });
    res.send("<script>alert('Sent!'); window.location.href='/dashboard';</script>");
});

app.get('/remove-user', (req, res) => {
    if (req.session.user !== 'admin') return res.redirect('/');
    usersTable = usersTable.filter(u => u.id !== parseInt(req.query.id));
    res.redirect('/dashboard');
});

app.post('/update-user-percentage', (req, res) => {
    if (req.session.user !== 'admin') return res.redirect('/');
    const user = usersTable.find(u => u.username === req.body.username);
    if(user) user.earnings_percentage = parseFloat(req.body.percentage);
    res.redirect('/dashboard');
});

app.post('/update-global-percentage', (req, res) => {
    if (req.session.user !== 'admin') return res.redirect('/');
    dbSaveSetting('global_earnings_percentage', req.body.global_percentage);
    res.redirect('/dashboard');
});

app.post('/save-sheet-config', (req, res) => {
    if (req.session.user !== 'admin') return res.redirect('/');
    dbSaveSetting('google_sheet_config', req.body.sheet_config);
    res.redirect('/dashboard');
});

app.post('/add-cpa', (req, res) => {
    if (req.session.user !== 'admin') return res.redirect('/');
    cpaConfigsTable.push({ id: cpaConfigsTable.length + 1, ...req.body, is_active: 1 });
    res.redirect('/dashboard');
});

app.get('/remove-cpa', (req, res) => {
    if (req.session.user !== 'admin') return res.redirect('/');
    cpaConfigsTable = cpaConfigsTable.filter(c => c.id !== parseInt(req.query.id));
    res.redirect('/dashboard');
});
