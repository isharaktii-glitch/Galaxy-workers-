...
async function initDb() {
    ...
    const settings = [
        ...
        ['gmail_task_instructions_en', 'Create a new Gmail account.'],
        ['gmail_task_instructions_si', 'නව Gmail ගිණුමක් සාදන්න.'],
        ['gmail_task_instructions_ta', 'புதிய Gmail கணக்கை உருவாக்கவும்.'],
    ...
    if (!buyer.length) {
        await sql`INSERT INTO users (username, password, email, address, contact, balance_numeric) VALUES ('buyer', 'buyer123', 'buyer@galaxy.com', 'Address', '000000', 0)`;
    }
}
...
// Helper: generate user's permanent task code
...
async function generateUserCode(username, referredBy) {
    ...
    if (!referredBy) {
        const max = await sql`SELECT MAX(CAST(SUBSTRING(referral_code FROM '([0-9]+)') AS INTEGER)) as max_val FROM users WHERE referral_code IS NOT NULL AND referral_code LIKE '%-%' AND referral_code NOT LIKE '%/%'`;
        const next = (max[0]?.max_val || 0) + 1;
...
const translations = {
    en: {
        ...
        subText: "Complete tasks below.", logout: "Logout", forgot: "Forgot Password?",
        ...
        allPaymentsDone: "ALL PAYMENTS DONE", paymentProof: "Payment Proof", uploadProof: "Upload",
        done: "DONE", wrong: "WRONG", reason: "Reason", submitReason: "Send",
        paymentReady: "Payment Ready", search: "Search",
        viewGmails: "📧 Gmails", viewOtherTasks: "📋 Other Tasks", hide: "Hide", deleteTask: "Delete"
    },
    si: {
        title: "GALAXY WORKERS", login: "සේවක ඇතුල්වීම", reg: "ලියාපදිංචිය",
        user: "පරිශීලක", pass: "මුරපද", email: "ඊමේල්", addr: "ලිපිනය", phone: "දුරකථන",
        ...
        buyerLogin: "ගැනුම්කරු", buyerDashboard: "Buyer Dashboard", buyerWelcome: "ආයුබෝවන්",
        ...
        viewGmails: "📧 Gmails", viewOtherTasks: "📋 වෙනත්", hide: "සඟවන්න", deleteTask: "මකන්න"
    },
    ta: {
        ...
        gmailHistory: "Gmail வரலாறு", referralEarnings: "பரிந்துரை",
        buyerLogin: "வாங்குபவர்", buyerDashboard: "Buyer Dashboard", buyerWelcome: "வரவேற்கிறோம்",
        ...
        viewGmails: "📧 Gmails", viewOtherTasks: "📋 மற்றவை", hide: "மறை", deleteTask: "நீக்கு"
    }
};
...
app.get('/buyer-dashboard', async (req, res) => {
    ...
    try {
        ...
        if (!tasks.length) tasksHtml += '<p>No Gmail tasks yet.</p>';
            ...
            const grouped = {};
            tasks.forEach(x => {
                if (!grouped[x.task_code]) grouped[x.task_code] = [];
                grouped[x.task_code].push(x);
            });
...
app.get('/buyer-mark-done', async (req, res) => {
    ...
    try {
        const task = await sql`SELECT * FROM gmail_tasks WHERE id = ${id} AND status='Pending'`;
        if (task.length) {
            const t = task[0];
            await sql`UPDATE gmail_tasks SET status='Success' WHERE id = ${id}`;
            ...
            if (t.referral_commission_paid === 0) {
                ...
                if (user.length && user[0].referred_by) {
                    ...
                    await sql`UPDATE gmail_tasks SET referral_commission_paid=1 WHERE id = ${id}`;
...
app.post('/buyer-mark-wrong', async (req, res) => {
    ...
    try {
        ...
        const t = await sql`SELECT username FROM gmail_tasks WHERE id = ${task_id}`;
        if (t.length) {
            await sql`INSERT INTO notifications (target_user, message, timestamp) VALUES (${t[0].username}, ${'❌ Gmail rejected: '+reason}, ${new Date().toLocaleString()})`;
...
app.get('/buyer-mark-payment-ready', async (req, res) => {
    if (req.session.user !== 'buyer') return res.redirect('/');
    try {
        const t = await sql`UPDATE gmail_tasks SET status='PaymentReady' WHERE id = ${req.query.id} AND status='Success' RETURNING *`;
...
app.get('/dashboard', async (req, res) => {
    ...
    try {
        if (username === 'admin') {
            ...
            let userMetricsHtml = `<h3>👥 Workers</h3>
            ...
            filteredUsers.forEach(u => {
                ...
                const gmailDetails = userGmails.map(g => `<div style="margin:5px 0;font-size:12px">📧 ${g.email_created} (${g.task_code}) - ${g.status} $${g.amount}</div>`).join('') || 'No Gmails';
                const otherDetails = userLogs.map(l => `<div style="margin:5px 0;font-size:12px">• ${l.task_name} - ${l.status} $${l.amount} <small>(${l.timestamp})</small></div>`).join('') || 'No tasks';
                userMetricsHtml += `<div class="user-row">
                    ...
                    <div id="other-${u.username}" style="display:none;margin-top:10px">${otherDetails}</div>
                </div>`;
            });
            ...
            res.send(htmlWrapper(req, 'Admin Dashboard', `
                ...
                <div class="navbar">
                    ...
                    <button class="nav-tab" onclick="switchSection('ref-settings')">💰 Referral</button>
                </div>
                ...
                <div id="users" class="dashboard-section">${userMetricsHtml}</div>
                ...
                <div id="ref-settings" class="dashboard-section">
            ...
            const gmailHistoryHtml = gmailLogs.length === 0 ? '<p>No Gmail tasks yet.</p>' : gmailLogs.map(g => {
                const delBtn = g.status === 'Pending' ? `<a href="/delete-gmail-task?id=${g.id}" onclick="return confirm('Delete?')" style="color:#ff4d4d;font-size:12px">${t.deleteTask}</a>` : '';
            ...
            res.send(htmlWrapper(req, 'Worker Dashboard', `
                ...
                <div class="navbar">
                    ...
                    <button class="nav-tab" onclick="switchSection('worker-notifs')">🔔 Alerts ${unread[0].c>0?`<span style="background:red;color:white;padding:2px 5px;border-radius:50%">${unread[0].c}</span>`:''}</button>
...
// Delete own Gmail (pending only)
app.get('/delete-gmail-task', async (req, res) => {
    if (!req.session.user || ['admin','buyer'].includes(req.session.user)) return res.redirect('/');
    const id = req.query.id;
    await sql`DELETE FROM gmail_tasks WHERE id = ${id} AND username = ${req.session.user} AND status = 'Pending'`;
...
app.post('/update-referral-settings', async (req, res) => {
    ...
    res.send("<script>alert('Updated!'); location.href='/dashboard?tab=ref-settings'</script>");
});
